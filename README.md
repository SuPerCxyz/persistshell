...
- `persist-holder` 持有 PTY、replay buffer 和日志 writer。
- Shell 是 Holder 的子进程；daemon 重启不应终止 Holder 和 Shell。
- SSH Client 只负责输入输出。
- SSH 断开只会断开 Client。
...
普通用户只需阅读完整手册：`docs/user/USER_GUIDE.md`。发行包安装后的路径为：

```text
/usr/share/doc/persistshell/USER_GUIDE.md
```

release 包同时将内部 PTY Holder 固定安装到：
