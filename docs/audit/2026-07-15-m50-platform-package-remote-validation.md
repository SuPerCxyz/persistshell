...
Ubuntu 26.04 与 RHEL 9 x86_64 打包流程已在本机和全新 `rockylinux:9` 容器中完成验证。
RHEL 9 RPM 已安装到 test 的 Rocky Linux 9.7，文档功能完成黑盒验证。新 GitHub workflow
尚未推送触发，因此不能把 hosted runner job 或新 artifact 标记为已通过。
...
- GitHub `ubuntu-26.04` 当前是 Preview runner；实际 hosted job 必须在推送后单独确认。
