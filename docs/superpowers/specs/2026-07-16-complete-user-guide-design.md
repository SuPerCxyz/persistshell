...
5. 指定 Session、Closed Session 冷恢复、多电脑 writer takeover 和 readonly attach。
6. SSH 自动接管、非交互兼容、临时绕过和永久卸载。
7. 全部 CLI 命令及常用参数，按任务分组并提供示例。
8. 日志查看、搜索、导出、replay 及日志边界。
9. rename、note、tag、pin、lock 等 Session 管理能力。
10. ps、stats、snapshot、metrics 等观测能力。
11. 配置、目录、权限、环境变量恢复和 SSH agent 同步边界。
12. daemon 管理、升级、卸载、保留或清理数据。
13. 常见工作流、故障排查、已知限制和命令速查表。
