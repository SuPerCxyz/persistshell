...
CPU 和 I/O 速率根据相邻点差值计算；第一点或计数器回退时通过 `rates_available = false` 显示
暂无数据，不能以零值伪装。CPU 按整台机器容量展示，多核任务允许超过 `100%`。Session RSS
是进程树 RSS 之和，共享页可能重复计算。进程退出或字段不可读时使用部分采集状态。
