...
在 `persistd` 中增加单一有界采样器：

- 每 5 秒扫描一次 `/proc`，一次扫描聚合所有活跃 Session 进程树。
...
组件。单采样器可以复用一次 `/proc` 扫描，并集中实施时间、内存和磁盘上限。
...
- [ ] 定义版本化指标记录和 Dashboard IPC 编码。
- [ ] 实现有界采样、聚合、轮转和恢复。
- [ ] 实现 `persist top` 与伪终端测试。
- [ ] 完成 100/1000 Session benchmark 和用户文档。
