...
- [x] test Rocky Linux 9.7 原生构建 rpm，验证 checksum、二进制、压缩 man page、三种 completion
  及用户文档路径。
- [x] M51 已验证 tar、deb 和 rpm 均携带 `USER_GUIDE.md`，且安装路径可直接阅读。
...
- [x] `v0.1.0` 已推送并同步到 GitHub mirror；tag CI run `29414016648` 和 Package run
  `29414016642` 均通过。
- [x] 下载 workflow artifact，独立执行 `sha256sum --check`，检查 tarball、deb、rpm 的版本、架构、
...
尚未创建 GitHub Release、签名或 SBOM。`v0.1.0` 历史 tag 保持不变；tag 后平台修复、mirror
同步、Package workflow、artifact 下载复核和 test 安装验证均已完成。
